...
export function ToolResultTray({ fileName, fileSize, metrics = [], primaryAction, secondaryActions = [], status }: ToolResultTrayProps) {
    ...
    return (
        <div className="pointer-events-none absolute inset-x-3 bottom-3 z-40 flex justify-center lg:bottom-4">
            <div className="pointer-events-auto flex max-w-[min(760px,calc(100vw-2rem))] flex-col gap-2 rounded-[8px] border border-white/10 bg-app-surface/92 p-2 shadow-result-tray backdrop-blur-xl">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                    {fileName && (
                        <div className="flex min-w-0 items-center gap-2 rounded-[6px] border border-white/8 bg-white/[0.03] px-2.5 py-1.5">
                            ...
                            <span className="max-w-[220px] truncate text-[11px] font-medium text-neutral-200">{fileName}</span>
                ...
                {(metrics.length > 0 || primaryAction || secondaryActions.length > 0) && (
