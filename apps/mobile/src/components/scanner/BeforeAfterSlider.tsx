import { Canvas, Circle, Fill, Group, Image, Rect, Text as SkiaText, matchFont, rect, useImage } from "@shopify/react-native-skia";
        ...
        <View style={[styles.container, { aspectRatio }]}>
            <GestureDetector gesture={gesture}>
                <Canvas style={styles.canvas} onSize={canvasSize}>
                    ...
                    {imagesReady ? (
                        <>
                            <Image image={afterImage} x={0} y={0} width={canvasWidth} height={canvasHeight} fit="contain" />
                            <Group clip={beforeClip}>
                                <Image image={beforeImage} x={0} y={0} width={canvasWidth} height={canvasHeight} fit="contain" />
                            </Group>
                            <Rect x={dividerLineX} y={0} width={2} height={canvasHeight} color={DIVIDER_COLOR} />
                            <Circle cx={dividerX} cy={handleCenterY} r={HANDLE_RADIUS} color={DIVIDER_COLOR} />
                            <SkiaText x={handleLabelX} y={handleLabelY} text="<>" font={HANDLE_FONT} color="#101214" />
                        </>
                    ) : null}
                </Canvas>
            </GestureDetector>
