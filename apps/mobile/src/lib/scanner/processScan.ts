...
import { prepareE2eInputTensor, prepareInputTensor, remapAndSave, saveRectifiedTensor } from "@/lib/native/documentRectifier";
import { getMaxProcessingLongEdge, getSelectedDocScannerModelId } from "@/lib/storage/scannerSettings.repository";
import { copyFallbackRectifiedImage, copyOriginalImage, getRectifiedImageUri } from "@/lib/storage/scanFiles";
...
export async function processScan(sourceUri: string, source: ScanSource): Promise<ProcessScanResult> {
    ...
    const originalUri = await measureStep("copy original", timings, () => copyOriginalImage(sourceUri, scanId));
    ...
    try {
        if (modelMode === "e2e") {
            const prepared = await measureStep("prepare e2e tensor", timings, () => prepareE2eInputTensor(originalUri, maxProcessingLongEdge));
            width = prepared.width;
            height = prepared.height;

            const { runDocScannerE2e } = await import("@/lib/model/docscannerModel");
            const rectified = await measureStep("onnx inference", timings, () => runDocScannerE2e(prepared.input, prepared.width, prepared.height, modelId));
            ...
            const prepared = await measureStep("prepare tensor", timings, () => prepareInputTensor(originalUri, maxProcessingLongEdge));
            width = prepared.width;
            height = prepared.height;

            const { runDocScannerFlow } = await import("@/lib/model/docscannerModel");
            const flow = await measureStep("onnx inference", timings, () => runDocScannerFlow(prepared.input, modelId));
        ...
        rectifiedUri = await measureStep("fallback copy", timings, () => copyFallbackRectifiedImage(originalUri, scanId));
    }
