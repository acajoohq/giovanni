...
import { DOCSCANNER_FLOW_LENGTH, DOCSCANNER_TENSOR_LENGTH } from "@/lib/scanner/scan.constants";
...
describe("DocScanner tensor validation", () => {
    ...
    it("rejects malformed model input tensors", () => {
        expect(() => assertDocScannerInput(new Float32Array(DOCSCANNER_TENSOR_LENGTH - 1))).toThrow("Expected DocScanner input");
    });
    ...
    it("rejects malformed flow outputs", () => {
        expect(() => assertDocScannerFlow(new Float32Array(DOCSCANNER_FLOW_LENGTH - 1))).toThrow("Expected DocScanner output");
    });
});
