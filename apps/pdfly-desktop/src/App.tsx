  ...
  return (
    <>
      <button
        ...
        onDragEnter={(e) => { e.preventDefault(); if (isFileDrag(e.dataTransfer)) setDragOver(true); }}
        onDragOver={(e) => { e.preventDefault(); if (isFileDrag(e.dataTransfer)) { e.dataTransfer.dropEffect = "copy"; setDragOver(true); } else setDragOver(false); }}
        onDragLeave={(e) => { const next = e.relatedTarget as Node | null; if (!next || !e.currentTarget.contains(next)) setDragOver(false); }}
        onDrop={handleDrop}
      >
        ...
        <span className="upload-title">
          <span className="upload-title--default">
            {multipleFiles ? "Drop PDFs here or click to add files" : "Drop PDF here or click to upload"}
          </span>
        ...
        <div className="upload-loader" aria-hidden={!isLoading}>
          ...
          <span className="upload-loader-label" aria-live="polite">{loaderLabel}</span>
        </div>
      </button>
      <input
        ref={inputRef}
...
function CompressTab() {
  ...
  const handleFile = useCallback(async (file: File) => {
    if (!isPdfFile(file)) { setStatus({ message: "Please select a PDF file", type: "error" }); return; }
    ...
    <section className="tab-panel active">
      ...
      <div className="options">
        <label className="option-group">
          <span>Compression Level: <strong>{options.compressionLevel}</strong></span>
          <input type="range" min={1} max={9} value={options.compressionLevel} onChange={(e) => setOptions((o) => ({ ...o, compressionLevel: Number(e.target.value) }))} />
        ...
        <div className="option-group">
          ...
          <label className="checkbox-label"><input type="checkbox" checked={options.recompressFlate} onChange={(e) => setOptions((o) => ({ ...o, recompressFlate: e.target.checked }))} /> Recompress flate streams</label>
          <label className="checkbox-label"><input type="checkbox" checked={options.compressPages} onChange={(e) => setOptions((o) => ({ ...o, compressPages: e.target.checked }))} /> Compress pages</label>
          <label className="checkbox-label"><input type="checkbox" checked={options.removeUnreferencedResources} onChange={(e) => setOptions((o) => ({ ...o, removeUnreferencedResources: e.target.checked }))} /> Remove unreferenced resources</label>
        </div>
      </div>
      ...
      {result && (
        <div className="results show">
          ...
          <div className="stats">
            <div className="stat"><div className="stat-label">Original Size</div><div className="stat-value">{formatBytes(result.originalSize)}</div></div>
            <div className="stat"><div className="stat-label">Compressed Size</div><div className="stat-value">{formatBytes(result.compressedSize)}</div></div>
            <div className="stat"><div className="stat-label">Saved</div><div className="stat-value">{formatBytes(result.savedBytes)}</div></div>
            <div className="stat"><div className="stat-label">Processing Time</div><div className="stat-value">{result.processingTime}</div></div>
            <div className="stat"><div className="stat-label">Throughput</div><div className="stat-value">{result.throughput}</div></div>
          </div>
          <button className="button" type="button" onClick={() => downloadData(result.data, "compressed.pdf")}>Download Compressed PDF</button>
        </div>
      )}
    </section>
  );
...
function SplitTab() {
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    result.pages.forEach((p, i) => { entries[`${result.baseName}_page_${i + 1}.pdf`] = p; });
    try {
      const zipped = await new Promise<Uint8Array>((resolve, reject) => {
        zip(entries, { level: 0 }, (err, data) => { if (err) reject(err); else resolve(data); });
      });
      ...
      {result && (
        <div className="split-results show">
          <div className="split-header">
            <h2>{result.pageCount} {result.pageCount === 1 ? "page" : "pages"} extracted</h2>
            <button className="button secondary" type="button" onClick={handleDownloadAll}>Download ZIP</button>
          </div>
          <div className="stats split-speed-stats">
            <div className="stat"><div className="stat-label">Processing Time</div><div className="stat-value">{result.processingTime}</div></div>
            <div className="stat"><div className="stat-label">Pages / second</div><div className="stat-value">{result.pps}</div></div>
            <div className="stat"><div className="stat-label">Input throughput</div><div className="stat-value">{result.throughput}</div></div>
          </div>
            ...
            {visiblePages.map((page, i) => {
              ...
              return (
                <div key={idx} className="page-item">
                  ...
                  <button className="button secondary small" type="button" onClick={() => downloadData(page, fileName)}>Download</button>
                </div>
              );
            })}
          </div>
          {total > SPLIT_PAGE_SIZE && (
            <div className="split-pagination">
              <button className="button secondary small" type="button" disabled={safeIndex <= 0} onClick={() => setPageIndex((i) => i - 1)}>Previous</button>
              <span className="split-page-info">Showing {sliceStart + 1}–{sliceEnd} of {total} · list {safeIndex + 1} / {pageCount}</span>
              <button className="button secondary small" type="button" disabled={safeIndex >= pageCount - 1} onClick={() => setPageIndex((i) => i + 1)}>Next</button>
            </div>
          )}
        </div>
      )}
...
function MergeTab() {
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    ...
    if (pdfs.length === 0) { setStatus({ message: "Please select PDF files only", type: "error" }); return; }
  ...
  const handleMerge = useCallback(async () => {
    if (files.length < 2) { setStatus({ message: "Please add at least 2 PDFs to merge", type: "error" }); return; }
    setLoading(true);
    setStatus({ message: "", type: "" });
    setResult(null);
    try {
  ...
  return (
    <section className="tab-panel active">
      <UploadArea id="merge-input" copy="Select multiple PDFs to merge into one" isLoading={false} loaderLabel="Working…" onFile={(f) => addFiles([f])} multipleFiles onFiles={addFiles} />
          ...
          <div className="merge-file-list">
            {files.map((file, i) => (
              <div key={i} className="page-item">
                ...
                <button className="button secondary small" type="button" onClick={() => removeFile(i)}>Remove</button>
              </div>
            ))}
          </div>
          <div className="merge-actions">
            <span className="merge-actions-hint">{files.length < 2 ? "Add at least one more PDF" : `${files.length} files ready`}</span>
            <button className="button" type="button" disabled={files.length < 2 || loading} aria-busy={loading} onClick={handleMerge}>Merge PDFs</button>
          </div>
        </>
      )}
      ...
      {result && (
        <div className="results show">
          <h2>Merge Results</h2>
          <div className="stats">
            <div className="stat"><div className="stat-label">Output Size</div><div className="stat-value">{result.outputSize}</div></div>
            <div className="stat"><div className="stat-label">Files Merged</div><div className="stat-value">{String(result.fileCount)}</div></div>
            <div className="stat"><div className="stat-label">Processing Time</div><div className="stat-value">{result.processingTime}</div></div>
            <div className="stat"><div className="stat-label">Throughput</div><div className="stat-value">{result.throughput}</div></div>
          </div>
          <button className="button" type="button" onClick={() => downloadData(result.data, "merged.pdf")}>Download Merged PDF</button>
        </div>
      )}
    </section>
  );
  ...
  return (
    <>
      ...
      <footer className="footer">
        <p>Powered by <a href="https://github.com/qpdf/qpdf" rel="noreferrer" target="_blank">qpdf</a> compiled to WebAssembly</p>
        <p>{version}</p>
      </footer>
    </>
  );
