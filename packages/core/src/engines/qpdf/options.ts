...
import type { DecodeLevel, ObjectStreamMode, OptimizeOptions, QpdfOptimizePreset, WriteOptions } from "../../types/qpdf.types.js";
import type { WasmCompressionOptions } from "../../types/wasm.types.js";

export const QPDF_PRESETS = {
    default: {
        compressionLevel: 6,
...
export function validateQpdfOptions(options?: OptimizeOptions): WasmCompressionOptions {
    const presetName = options?.preset ?? "default";
    const preset = QPDF_PRESETS[presetName];

    if (!preset) {
        throw new QpdfValidationError(`preset must be one of: ${Object.keys(QPDF_PRESETS).join(", ")}`);
    }

    return validateQpdfWriteOptions({ ...preset, ...options });
}

export function validateQpdfWriteOptions(options?: WriteOptions): WasmCompressionOptions {
    const result: WasmCompressionOptions = { ...QPDF_PRESETS.default };
...
export function getQpdfPreset(options?: OptimizeOptions): QpdfOptimizePreset {
