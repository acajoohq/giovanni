const { createRunOncePlugin, withAppBuildGradle, withMainApplication, withSettingsGradle } = require("@expo/config-plugins");
...
const dependencyLines = ['    implementation project(":onnxruntime-react-native")'];

const importLines = ["import ai.onnxruntime.reactnative.OnnxruntimePackage"];

const packageLines = ["          add(OnnxruntimePackage())"];
...
function withDocumentScannerAndroidSettings(config) {
    return withSettingsGradle(config, (config) => {
        config.modResults.contents = insertMissingLinesAfter(config.modResults.contents, "include ':app'", settingsLines);
        return config;
    });
}
...
function withDocumentScannerAndroidBuild(config) {
    return withAppBuildGradle(config, (config) => {
        ...
        config.modResults.contents = insertMissingLinesAfter(config.modResults.contents, '    implementation("com.facebook.react:react-android")', dependencyLines);
        return config;
    });
}
...
function withDocumentScannerMainApplication(config) {
    return withMainApplication(config, (config) => {
        ...
        let contents = insertMissingLinesAfter(config.modResults.contents, "import com.facebook.react.defaults.DefaultNewArchitectureEntryPoint", importLines);

        contents = insertMissingLinesAfter(contents, "          // add(MyReactNativePackage())", packageLines);
...
module.exports = createRunOncePlugin(withDocumentScannerNativeModules, pkg.name, pkg.version);
