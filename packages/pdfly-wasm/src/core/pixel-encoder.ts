...
export async function encodeRawPixelsAsPng(image: WasmExtractedImage): Promise<Blob | null> {
    ...
    const rgba = expandToRgba(image.bytes, image.width, image.height, components);
    if (!rgba) return null;
...
function expandToRgba(pixels: Uint8Array, width: number, height: number, components: number): Uint8ClampedArray | null {
    ...
    if (components === 3) {
        // sRGB passthrough — embedded ICC profile (if any) is ignored
        for (let i = 0; i < total; i++) {
            const source = i * 3;
            const offset = i * 4;
    ...
    if (components === 4) {
        // naive CMYK→RGB without color management — preview-quality only
        for (let i = 0; i < total; i++) {
            const source = i * 4;
