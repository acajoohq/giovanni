...
describe("Validation Utilities", () => {
  describe("normalizeBuffer", () => {
    ...
    it("should throw on invalid input", () => {
      expect(() => normalizeBuffer("invalid" as unknown as Uint8Array)).toThrow(
        QpdfValidationError,
      );
    });
  });
  ...
  describe("validateCompressionOptions", () => {
    ...
    it("should reject invalid compression level", () => {
      expect(() => validateCompressionOptions({ compressionLevel: 0 })).toThrow(
        QpdfValidationError,
      );

      expect(() => validateCompressionOptions({ compressionLevel: 10 })).toThrow(
        QpdfValidationError,
      );

      expect(() => validateCompressionOptions({ compressionLevel: 5.5 })).toThrow(
        QpdfValidationError,
      );
    });
    ...
    it("should reject invalid decode level", () => {
      expect(() =>
        validateCompressionOptions({ decodeLevel: "invalid" } as unknown as CompressionOptions),
