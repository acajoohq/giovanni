...
export class QPDFWriter {
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    try {
      ...
      throw new QpdfCompressionError(
        'Failed to initialize QPDFWriter',
        { cause: error }
      );
    }
  }
  ...
  async setDecodeLevel(level: DecodeLevel): Promise<void> {
    ...
    if (!validLevels.includes(level)) {
      throw new QpdfValidationError(
        `Decode level must be one of: ${validLevels.join(', ')}`
      );
    }
    await this.ensureInitialized();
  ...
  async setObjectStreamMode(mode: ObjectStreamMode): Promise<void> {
    ...
    if (!validModes.includes(mode)) {
      throw new QpdfValidationError(
        `Object stream mode must be one of: ${validModes.join(', ')}`
      );
    }
    await this.ensureInitialized();
  ...
  getBuffer(): Uint8Array {
    if (!this.initialized || !this.wasmInstance) {
      throw new QpdfValidationError(
        'Writer not initialized. Call write() first.'
      );
    }
    try {
