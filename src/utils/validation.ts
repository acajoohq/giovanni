...
export function validateCompressionOptions(
  ...
  if (options.compressionLevel !== undefined) {
    if (
      ...
      throw new QpdfValidationError(
        'compressionLevel must be an integer between 1 and 9'
      );
    }
  ...
  if (options.decodeLevel !== undefined) {
    const validDecodeLevels: DecodeLevel[] = [
      'none',
      'generalized',
      'specialized',
      'all',
    ...
    if (!validDecodeLevels.includes(options.decodeLevel)) {
      throw new QpdfValidationError(
        `decodeLevel must be one of: ${validDecodeLevels.join(', ')}`
      );
    }
  ...
  if (options.objectStreams !== undefined) {
    ...
    if (!validModes.includes(options.objectStreams)) {
      throw new QpdfValidationError(
        `objectStreams must be one of: ${validModes.join(', ')}`
      );
    }
  ...
  if (options.removeUnreferencedResources !== undefined) {
    result.removeUnreferencedResources = Boolean(
      options.removeUnreferencedResources
    );
  }
