...
import { QpdfValidationError } from '../src/core/errors.js';
...
describe('Validation Utilities', () => {
  describe('normalizeBuffer', () => {
    ...
    it('should throw on invalid input', () => {
      expect(() => normalizeBuffer('invalid' as any)).toThrow(
        QpdfValidationError
      );
    });
  });
  ...
  describe('validateCompressionOptions', () => {
    ...
    it('should reject invalid compression level', () => {
      expect(() =>
        validateCompressionOptions({ compressionLevel: 0 })
      ).toThrow(QpdfValidationError);

      expect(() =>
        validateCompressionOptions({ compressionLevel: 10 })
      ).toThrow(QpdfValidationError);

      expect(() =>
        validateCompressionOptions({ compressionLevel: 5.5 })
      ).toThrow(QpdfValidationError);
    });
    ...
    it('should reject invalid decode level', () => {
      expect(() =>
        validateCompressionOptions({ decodeLevel: 'invalid' as any })
      ).toThrow(QpdfValidationError);
    });
    ...
    it('should reject invalid object streams mode', () => {
      expect(() =>
        validateCompressionOptions({ objectStreams: 'invalid' as any })
      ).toThrow(QpdfValidationError);
    });

    it('should coerce boolean values', () => {
      const result = validateCompressionOptions({
        recompressFlate: 1 as any,
        compressPages: 'true' as any,
      });
